...
//        String fingerprint = "2192328486";
    ...
    public Observable<List<KeDouModel>> videoList(String category, int page, boolean pullToRefresh) {
        String bid = "list_videos_common_videos_list";
        String sort = "video_viewed";
        if (category.equals("latest-updates")) {
            bid = "list_videos_latest_videos_list";
            sort = "post_date";
        } else if (category.equals("top-rated")) {
            sort = "rating";
        }
        return keDouServiceApi.videoList(category, bid, sort, page).map(ParseKeDouWo::parseVideoList);
    }
