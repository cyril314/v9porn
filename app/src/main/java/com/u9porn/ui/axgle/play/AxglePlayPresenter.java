...
public class AxglePlayPresenter extends MvpBasePresenter<AxglePlayView> implements IAxglePlay {
	...
	public void getPlayVideoUrl(String vid) {
		...
		String axgleAddress = dataManager.getAxgleAddress().replace("api.", "");
		//如果用户忘记输入最后一个“/”，我们手动加上去不就行了，或者设置的时候直接加上就ok了
		if (!axgleAddress.endsWith("/")) {
			axgleAddress += "/";
		}
		String hashStr = PSVS21.computeHash(new PSVS21.StubContext(context), vid, ts);
		String url = String.format(axgleAddress + "mp4.php?vid=%s&ts=%s&hash=%s&m3u8", vid, ts, hashStr);
