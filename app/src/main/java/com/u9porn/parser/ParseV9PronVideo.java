...
//        Elements listchannel = body.getElementsByClass("listchannel");
    ...
    public static VideoResult parseVideoPlayUrl(String html, User user) {
        ...
        String videoId = myFileNameGenerator.generate(videoUrl);
        ...
        Logger.t(TAG).d("%n视频标题：$s,%n视频链接：%s,%n视频Id：%s ,作者Id: %s, 作者: %s, 添加时间：%s,%n缩略图：%s",
                        videoResult.getVideoName(), videoResult.getVideoUrl(), videoResult.getVideoId(), videoResult.getAuthorId(),
                        videoResult.getOwnerName(), videoResult.getAddDate(), videoResult.getThumbImgUrl());
