...
public class ParsePxgav {
    ...
    public static BaseResult<PxgavVideoParserJsonResult> parserVideoUrl(String html) {
        BaseResult<PxgavVideoParserJsonResult> baseResult = new BaseResult<PxgavVideoParserJsonResult>();
        Document document = Jsoup.parse(html);
        Element videoWrapper = document.getElementsByClass("penci-entry-content entry-content").first().selectFirst("source");
        String videoHtml = videoWrapper.html();
        Logger.t(TAG).d(videoHtml);
        String videoUrl = videoWrapper.attr("src");
...
//        PxgavVideoParserJsonResult pxgavVideoParserJsonResult = new Gson().fromJson(videoUrl, PxgavVideoParserJsonResult.class);
        ...
        Logger.t(TAG).d("Pxgav视频播放地址： %s",videoUrl);
        PxgavVideoParserJsonResult pvpr = new PxgavVideoParserJsonResult();
        pvpr.setFile(videoUrl);

        List<PxgavModel> pxgavModelList = new ArrayList<PxgavModel>();
        ...
        for (Element element : items) {
            ...
            String imgUrl = a.attr("data-src");
            pxgavModel.setImgUrl(imgUrl);
            int beginIndex = pxgavModel.getImgUrl().lastIndexOf("/");
            int endIndex = pxgavModel.getImgUrl().lastIndexOf("-");
            String pId = StringUtils.subString(pxgavModel.getImgUrl(), beginIndex + 1, endIndex);
            pxgavModel.setpId(pId);

            pxgavModelList.add(pxgavModel);
        }
        pvpr.setPxgavModelList(pxgavModelList);
        baseResult.setData(pvpr);
        return baseResult;
    }
